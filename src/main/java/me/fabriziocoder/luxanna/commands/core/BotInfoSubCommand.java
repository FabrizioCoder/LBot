...
public class BotInfoSubCommand extends SlashCommand {
    ...
    public void execute(SlashCommandEvent event) {
        ...
        EmbedBuilder messageEmbed = new EmbedBuilder().setTitle(String.format("%s - your League of Legends bot", event.getJDA().getSelfUser().getName()), "https://top.gg/bot/949565943275720736").setColor(0x2564f4).setFooter("Last restart", null).setTimestamp(event.getClient().getStartTime()).setThumbnail(event.getJDA().getSelfUser().getAvatarUrl()).setImage("https://i.imgur.com/XTgkUlF.png");
